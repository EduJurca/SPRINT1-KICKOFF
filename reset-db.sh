...
for i in {1..20}; do
    if docker exec VC-mariadb mariadb -u simsuser -p'simspass123.' -e "SELECT 1" &>/dev/null; then
...
if docker exec VC-mariadb mariadb -u simsuser -p'simspass123.' simsdb -e "DESCRIBE vehicles;" 2>/dev/null | grep -q "battery_level"; then
    echo "✅ Tabla 'vehicles' verificada correctamente"
    
    # Mostrar cuántos vehículos hay
    VEHICLE_COUNT=$(docker exec VC-mariadb mariadb -u simsuser -p'simspass123.' simsdb -e "SELECT COUNT(*) FROM vehicles;" 2>/dev/null | tail -1)
