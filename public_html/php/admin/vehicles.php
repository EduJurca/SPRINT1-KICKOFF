...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        switch ($action) {
            case 'add':
                // Add new vehicle
                $plate = $db->real_escape_string($_POST['plate']);
                ...
                $vehicle_type = $db->real_escape_string($_POST['vehicle_type']);
                $is_accessible = isset($_POST['is_accessible']) ? 1 : 0;
                $price_per_minute = floatval($_POST['price_per_minute']);
                $image_url = $db->real_escape_string($_POST['image_url'] ?? '');
                ...
                $sql = "INSERT INTO vehicles (plate, brand, model, year, battery_level, latitude, longitude, status, vehicle_type, is_accessible, accessibility_features, price_per_minute, image_url) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $db->prepare($sql);
                $stmt->bind_param('sssidddssisss', $plate, $brand, $model, $year, $battery_level, $latitude, $longitude, $status, $vehicle_type, $is_accessible, $accessibility_json, $price_per_minute, $image_url);
            ...
            case 'edit':
                ...
                $stmt = $db->prepare($sql);
                $stmt->bind_param('ssidddssisssi', $brand, $model, $year, $battery_level, $latitude, $longitude, $status, $vehicle_type, $is_accessible, $accessibility_json, $price_per_minute, $image_url, $id);
            ...
            case 'bulk_status':
                ...
                $new_status = $db->real_escape_string($_POST['bulk_status']);
