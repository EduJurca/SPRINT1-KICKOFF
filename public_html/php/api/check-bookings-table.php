...
try {
    ...
    $result['table_exists'] = $checkTable->num_rows > 0;
    
    if ($result['table_exists']) {
        // Get table structure
        $structure = $db->query("DESCRIBE bookings");
        $result['table_structure'] = $structure->fetch_all(MYSQLI_ASSOC);
    } else {
        // Create table
    ...
    if ($db->query($testSQL)) {
        ...
        $result['test_booking_id'] = $db->insert_id;
        
        // Delete test booking
        $db->query("DELETE FROM bookings WHERE id = " . $db->insert_id);
    } else {
