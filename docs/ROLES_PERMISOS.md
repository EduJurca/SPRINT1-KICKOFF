...
```php
// Verificar autenticació (API o navegador)
$userId = AuthController::requireAuth();

// Verificar si és admin (API o navegador)
$userId = AuthController::requireAdmin();

// Detecta automàticament:
// - API: retorna JSON amb error
// - Navegador: redirigeix a login/dashboard
```

### Permissions (Verificació de Rols)
```php
// Verificar permís
...
class UserController {
    public function __construct() {
        ...
        // Verificar rol (Staff = SuperAdmin o Treballador)
...
1. **Tots els controllers admin** han de verificar autenticació i rol al constructor
...
5. **`AuthController::requireAuth()`** detecta automàticament API vs navegador
