...
(function() {
    ...
    if (!window.API) {
        window.API = {
            ...
            async request(endpoint, options = {}) {
                ...
                try {
                    const response = await fetch(`${window.APP_CONFIG.apiBaseUrl}${endpoint}`, config);
    ...
    if (!window.SessionManager) {
        window.SessionManager = {
            ...
            async checkSession() {
                try {
                    const response = await window.API.get('/dashboard.php');
                    if (!response.success) {
            ...
            handleSessionExpired() {
                ...
                window.Utils.showToast('La teva sessió ha expirat', 'warning', 5000);
            ...
            setupActivityListeners() {
                ...
                events.forEach(event => {
                    document.addEventListener(event, window.Utils.debounce(() => {
             ...
             */
            updateLastActivity() {
                window.Utils.setLocalStorage('lastActivity', Date.now());
            }
        };
    }
